...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    //此ip为电脑的ipv4地址,端口号为服务器端口号
                    socket = new Socket("192.168.2.224", 10016);
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1) {
                ...
                if (split[0].equals(localPort + "")) {
                    MyBean bean = new MyBean(split[1], 2, split[2], "我：");
                    list.add(bean);
                } else {
                    MyBean bean = new MyBean(split[1], 1, split[2], ("来自：" + split[0]));
                    list.add(bean);
                }
